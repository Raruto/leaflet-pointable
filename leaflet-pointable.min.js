...
e)};L.Map.prototype._handlePointableMouseClick=function(e){if(!this._isPointablePixel)return;this.fire("pointable_mouseclick",e)};L.GridLayer.include({onAdd:function(map){if(this.options.pointable){map.addPointableLayer(this);map.on("mousemove",this._pointableMouseTracker,this)}onAddGridProto.call(this,map)},onRemove:function(map){if(this.options.pointable){map.removePointableLayer(this);map.off("mousemove",this._pointableMouseTracker,this)}onRemoveGridProto.call(this,map)},isPointablePixel:function(){return!!this._isPointablePixel},
_pointableMouseTracker:function(e){var coords=this._getPointableTileCoords(e.latlng.lat,e.latlng.lng,this._map.getZoom());var img=this._getPointableTile(coords);var pixel=this._getPointablePixel(img,e.originalEvent);this._isPointablePixel=this._hasPointablePixelData(img,pixel)},_getPointableTileCoords:function(lat,lon,zoom){var DEG_TO_RAD=Math.PI/180;var xtile=parseInt(Math.floor((lon+180)/360*(1<<zoom)));var ytile=parseInt(Math.floor((1-Math.log(Math.tan(lat*DEG_TO_RAD)+1/Math.cos(lat*DEG_TO_RAD))/
Math.PI)/2*(1<<zoom)));return{x:xtile,y:ytile,z:zoom}},_getPointableTile:function(coords){if(!this._tiles)return;var key=this._tileCoordsToKey(coords);var tile=this._tiles[key];if(!tile)return;var img=tile.el.tagName.toLowerCase()==="img"?tile.el:tile.el.querySelector("img");if(!img)return;this._downloadPointableTile(img.src,this._onPointableTileLoaded);return img},_getPointablePixel:function(img,e){if(!img)return;var imgRect=img.getBoundingClientRect();var imgPos={pageY:(imgRect.top+window.scrollY).toFixed(0),
pageX:(imgRect.left+window.scrollX).toFixed(0)};var mousePos={x:e.pageX-imgPos.pageX,y:e.pageY-imgPos.pageY};return mousePos},_hasPointablePixelData:function(img,pixelPoint){if(!this.tileContext||!pixelPoint)return;var pixelData=this.tileContext.getImageData(pixelPoint.x,pixelPoint.y,1,1).data;var alpha=pixelData[3];var hasTileData=alpha!=0;return hasTileData},_downloadPointableTile:function(imageSrc,callback){if(!imageSrc)return;img=new Image;img.crossOrigin="Anonymous";img.addEventListener("load",
callback.bind(this,img),false);img.src=imageSrc},_onPointableTileLoaded:function(img){this.tileCanvas=document.createElement("canvas");this.tileContext=this.tileCanvas.getContext("2d");var tileSize=this.getTileSize();this.tileCanvas.width=tileSize.x;this.tileCanvas.height=tileSize.y;this.tileContext.drawImage(img,0,0)}})})();
